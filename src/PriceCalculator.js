	...
	async function insert() {
		...
		if (itemValue !== '' && qtValue !== '') {
			try {
				...
				const data = await fetch(`http://localhost:8080/product/getProductByName?itemCode=${itemValue}&quantity=${qtValue}&slow=3`);
				...
				<Table striped bordered hover variant="dark">
					...
					<tbody>
						<tr>
							<td></td>
							<td></td>
							<td><input required id='nameTxt' type={'text'} placeholder='item code' ref={item}></input></td>
